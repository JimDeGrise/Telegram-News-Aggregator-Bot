...
async def build_runtime():
    ...
    async def scheduled_fetch():
        ...
        if total_new == 0:
            return added_map
        lines = [f"Свежие новости: {total_new}"]
...
async def main():
    ...
    # Планировщик: передаём прямо корутину (без create_task)
    # ВАЖНО: setup_scheduler должен использовать AsyncIOScheduler и add_job(scheduled_fetch, 'interval', ...)
    setup_scheduler(scheduled_fetch, cfg["interval_minutes"])

    # Первый запуск (опционально можно обернуть в try/except для логирования)
...
if __name__ == "__main__":
    try:
        ...
        # Fallback блок обычно уже не нужен, но оставлен на случай запуска в окружении,
        # которое само запускает цикл (например, внутри некоторых REPL / ноутбуков)
